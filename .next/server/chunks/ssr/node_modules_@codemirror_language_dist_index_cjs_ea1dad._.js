...
"[project]/node_modules/@codemirror/language/dist/index.cjs [app-rsc] (ecmascript)": (function({ r: __turbopack_require__, f: __turbopack_module_context__, i: __turbopack_import__, s: __turbopack_esm__, v: __turbopack_export_value__, n: __turbopack_export_namespace__, c: __turbopack_cache__, M: __turbopack_modules__, l: __turbopack_load__, j: __turbopack_dynamic__, P: __turbopack_resolve_absolute_path__, U: __turbopack_relative_url__, R: __turbopack_resolve_module_id_path__, g: global, __dirname, x: __turbopack_external_require__, y: __turbopack_external_import__, m: module, e: exports, t: require }) { !function() {

'use strict';
var common = __turbopack_require__("[project]/node_modules/@lezer/common/dist/index.cjs [app-rsc] (ecmascript)");
var state = __turbopack_require__("[project]/node_modules/@codemirror/state/dist/index.cjs [app-rsc] (ecmascript)");
var view = __turbopack_require__("[project]/node_modules/@codemirror/view/dist/index.cjs [app-rsc] (ecmascript)");
var highlight = __turbopack_require__("[project]/node_modules/@lezer/highlight/dist/index.cjs [app-rsc] (ecmascript)");
...
*/ class Language {
    ...
    */ name = ""){
        ...
        if (!state.EditorState.prototype.hasOwnProperty("tree")) Object.defineProperty(state.EditorState.prototype, "tree", {
            get () {
    ...
    */ findRegions(state) {
        ...
        let explore = (tree, from)=>{
            ...
            for(let i = 0; i < tree.children.length; i++){
                ...
                if (ch instanceof common.Tree) explore(ch, tree.positions[i] + from);
            }
        };
...
function topNodeAt(state, pos, side) {
    ...
    if (!topLang || topLang.allowsNesting) {
        for(let node = tree; node; node = node.enter(pos, side, common.IterMode.ExcludeBuffers))if (node.type.isTop) tree = node;
    }
    return tree;
}
/**
...
function cutFragments(fragments, from, to) {
    return common.TreeFragment.applyChanges(fragments, [
        {
            fromA: from,
            toA: to,
            fromB: from,
...
*/ const language = state.Facet.define({
    ...
    enables: (language)=>[
            Language.state,
            parseWorker,
            view.EditorView.contentAttributes.compute([
                language
            ], (state)=>{
                let lang = state.facet(language);
...
*/ function getIndentation(context, pos) {
    if (context instanceof state.EditorState) context = new IndentContext(context);
...
*/ const foldEffect = state.StateEffect.define({
    map: mapRange
});
/**
State effect that unfolds the given range (if it was folded).
*/ const unfoldEffect = state.StateEffect.define({
    map: mapRange
});
    ...
    update (folded, tr) {
        ...
        for (let e of tr.effects){
            if (e.is(foldEffect) && !foldExists(folded, e.value.from, e.value.to)) {
                ...
                let widget = !preparePlaceholder ? foldWidget : view.Decoration.replace({
...
const foldWidget = view.Decoration.replace({
...
*/ function foldGutter(config = {}) {
    ...
    return [
        ...
        view.gutter({
            ...
            markers (view) {
                var _a;
                return ((_a = view.plugin(markers)) === null || _a === void 0 ? void 0 : _a.markers) || state.RangeSet.empty;
            },
            initialSpacer () {
...
*/ function syntaxHighlighting(highlighter, options) {
    ...
    if (highlighter instanceof HighlightStyle) {
        if (highlighter.module) ext.push(view.EditorView.styleModule.of(highlighter.module));
    ...
    if (options === null || options === void 0 ? void 0 : options.fallback) ext.push(fallbackHighlighter.of(highlighter));
        ...
        view.EditorView.darkTheme
    ], (state)=>{
        return state.facet(view.EditorView.darkTheme) == (themeType == "dark") ? [
            highlighter
        ] : [];
    }));
...
class TreeHighlighter {
    ...
    buildDeco(view$1, highlighters) {
        if (!highlighters || !this.tree.length) return view.Decoration.none;
        let builder = new state.RangeSetBuilder();
        for (let { from, to } of view$1.visibleRanges){
            highlight.highlightTree(this.tree, highlighters, (from, to, style)=>{
                builder.add(from, to, this.markCache[style] || (this.markCache[style] = view.Decoration.mark({
                    class: style
                })));
            }, from, to);
        }
...
*/ const defaultHighlightStyle = HighlightStyle.define([
    ...
    {
        tag: [
            highlight.tags.literal,
            highlight.tags.inserted
        ],
        color: "#164"
    },
    {
    ...
    {
        tag: [
            highlight.tags.special(highlight.tags.variableName),
...
const matchingMark = view.Decoration.mark({
...
*/ class StreamLanguage extends Language {
    constructor(parser){
        ...
        self = this;
        this.streamParser = p;
        this.stateAfter = new common.NodeProp({
            perNode: true
        });
...
class Parse {
    ...
    finishChunk() {
        ...
        tree = new common.Tree(tree.type, tree.children, tree.positions, tree.length, [
            [
...
function docID(data) {
    let type = common.NodeType.define({
        id: typeArray.length,
        name: "Document",
        props: [
...
const alwaysIsolate = state.Facet.define({
...
const marks = {
    rtl: view.Decoration.mark({
        class: "cm-iso",
        inclusive: true,
        attributes: {
            dir: "rtl"
        },
        bidiIsolate: view.Direction.RTL
    }),
    ltr: view.Decoration.mark({
        class: "cm-iso",
        inclusive: true,
        attributes: {
            dir: "ltr"
        },
        bidiIsolate: view.Direction.LTR
    }),
    auto: view.Decoration.mark({
        class: "cm-iso",
        inclusive: true,
        attributes: {
            dir: "auto"
...
//# sourceMappingURL=node_modules_%40codemirror_language_dist_index_cjs_ea1dad._.js.map