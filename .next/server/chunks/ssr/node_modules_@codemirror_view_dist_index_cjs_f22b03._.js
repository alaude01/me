...
"[project]/node_modules/@codemirror/view/dist/index.cjs [app-rsc] (ecmascript)": (function({ r: __turbopack_require__, f: __turbopack_module_context__, i: __turbopack_import__, s: __turbopack_esm__, v: __turbopack_export_value__, n: __turbopack_export_namespace__, c: __turbopack_cache__, M: __turbopack_modules__, l: __turbopack_load__, j: __turbopack_dynamic__, P: __turbopack_resolve_absolute_path__, U: __turbopack_relative_url__, R: __turbopack_resolve_module_id_path__, g: global, __dirname, x: __turbopack_external_require__, y: __turbopack_external_import__, m: module, e: exports, t: require }) { !function() {

'use strict';
var state = __turbopack_require__("[project]/node_modules/@codemirror/state/dist/index.cjs [app-rsc] (ecmascript)");
var styleMod = __turbopack_require__("[project]/node_modules/style-mod/dist/style-mod.cjs [app-rsc] (ecmascript)");
var w3cKeyname = __turbopack_require__("[project]/node_modules/w3c-keyname/index.cjs [app-rsc] (ecmascript)");
...
class WidgetView extends ContentView {
    ...
    get overrideDOMText() {
        if (this.length == 0) return state.Text.empty;
        let top = this;
...
function moveVisually(line, order, dir, start, forward) {
    ...
    if (nextSpan && nextIndex == spanEnd && nextSpan.level + (forward ? 0 : 1) < span.level) return state.EditorSelection.cursor(nextSpan.side(!forward, dir) + line.from, nextSpan.forward(forward, dir) ? 1 : -1, nextSpan.level);
...
const scrollIntoView = state.StateEffect.define({
    map: (t, ch)=>t.map(ch)
});
...
const editable = state.Facet.define({
...
function groupAt(state$1, pos, bias = 1) {
    ...
    if (line.length == 0) return state.EditorSelection.cursor(pos);
    if (linePos == 0) bias = 1;
    else if (linePos == line.length) bias = -1;
    let from = linePos, to = linePos;
    if (bias < 0) from = state.findClusterBreak(line.text, linePos, false);
    else to = state.findClusterBreak(line.text, linePos);
...
function posAtCoords(view, coords, precise, bias = -1) {
    ...
    for(let halfLine = view.viewState.heightOracle.textHeight / 2, bounced = false;;){
        ...
        if (block.type == exports.BlockType.Text) break;
        for(;;){
...
function blockAt(view, pos) {
    ...
    if (Array.isArray(line.type)) for (let l of line.type){
        if (l.to > pos || l.to == pos && (l.to == line.to || l.type == exports.BlockType.Text)) return l;
    }
    return line;
}
function moveToLineBoundary(view, start, forward, includeWrap) {
    ...
    if (coords) {
        ...
        if (pos != null) return state.EditorSelection.cursor(pos, forward ? -1 : 1);
    }
...
function byGroup(view, pos, start) {
    ...
    return (next)=>{
        ...
        if (cat == state.CharCategory.Space) cat = nextCat;
        return cat == nextCat;
    };
}
function moveVertically(view, start, forward, distance) {
    let startPos = start.head, dir = forward ? 1 : -1;
    if (startPos == (forward ? view.state.doc.length : 0)) return state.EditorSelection.cursor(startPos, start.assoc);
...
function applyDOMChange(view, domChange) {
    ...
    if (!change && domChange.typeOver && !sel.empty && newSel && newSel.main.empty) {
        ...
        if (newSel && change.insert.length == 2) newSel = state.EditorSelection.single(newSel.main.anchor - 1, newSel.main.head - 1);
        change = {
            from: sel.from,
            to: sel.to,
            insert: state.Text.of([
                " "
            ])
        };
        ...
        if (newSel) newSel = state.EditorSelection.single(newSel.main.anchor - 1, newSel.main.head - 1);
        change = {
            from: sel.from,
            to: sel.to,
            insert: state.Text.of([
    ...
    skipAtoms(sel) {
        ...
        for(let i = 0; i < sel.ranges.length; i++){
            ...
            if (range.empty) {
                ...
                if (pos != range.from) updated = state.EditorSelection.cursor(pos, -1);
            } else {
                let from = skipAtomicRanges(this.atoms, range.from, -1);
                let to = skipAtomicRanges(this.atoms, range.to, 1);
                if (from != range.from || to != range.to) updated = state.EditorSelection.range(range.from == range.anchor ? from : to, range.from == range.head ? from : to);
            }
            if (updated) {
...
function basicMouseSelection(view, event) {
    ...
    return {
        ...
        get (event, extend, multiple) {
            ...
            else return state.EditorSelection.create([
                range
            ]);
        }
    };
...
function removeRangeAround(sel, pos) {
    for(let i = 0; i < sel.ranges.length; i++){
        ...
        if (from <= pos && to >= pos) return state.EditorSelection.create(sel.ranges.slice(0, i).concat(sel.ranges.slice(i + 1)), sel.mainIndex == i ? 0 : sel.mainIndex - (sel.mainIndex > i ? 1 : 0));
    }
    return null;
}
handlers.dragstart = (view, event)=>{
    ...
    if (event.target.draggable) {
        ...
        if (cView && cView.isWidget) {
            ...
            if (from >= range.to || to <= range.from) range = state.EditorSelection.range(from, to);
        }
    }
    let { inputState } = view;
...
class ViewState {
    constructor(state$1){
        ...
        this.heightMap = HeightMap.empty().applyChanges(this.stateDeco, state.Text.empty, this.heightOracle.setDoc(state$1.doc), [
            new ChangedRange(0, 0, 0, state$1.doc.length)
        ]);
        for(let i = 0; i < 2; i++){
    ...
    measure(view) {
        ...
        if (measureContent) {
            ...
            for (let vp of this.viewports){
                ...
                this.heightMap = (refresh ? HeightMap.empty().applyChanges(this.stateDeco, state.Text.empty, this.heightOracle, [
                    new ChangedRange(0, 0, 0, view.state.doc.length)
    ...
    ensureLineGaps(current, mayMeasure) {
        ...
        if (this.defaultTextDirection != exports.Direction.LTR && !wrapping) return [];
        let gaps = [];
        ...
        let checkLine = (line)=>{
            if (line.length < doubleMargin || line.type != exports.BlockType.Text) return;
...
const darkTheme = state.Facet.define({
...
class EditContextManager {
    constructor(view){
        ...
        this.handlers.textupdate = (e)=>{
            ...
            if (!view.state.readOnly) applyDOMChangeInner(view, change, state.EditorSelection.single(this.toEditorPos(e.selectionStart), this.toEditorPos(e.selectionEnd)));
...
*/ class EditorView {
    ...
    */ constructor(config = {}){
    ...
    dispatch(...input) {
        let trs = input.length == 1 && input[0] instanceof state.Transaction ? input : input.length == 1 && Array.isArray(input[0]) ? input[0] : [
    ...
    */ update(transactions) {
        ...
        for (let tr of transactions){
            if (tr.startState != state$1) throw new RangeError("Trying to update state with a transaction that doesn't start from the previous state.");
            state$1 = tr.state;
        }
        if (this.destroyed) {
        ...
        if (pendingKey) {
            ...
            if (domChange && !this.state.doc.eq(state$1.doc) || !this.state.selection.eq(state$1.selection)) domChange = null;
        } else {
            this.observer.clear();
        }
        // When the phrases change, redraw the editor
        if (state$1.facet(state.EditorState.phrases) != this.state.facet(state.EditorState.phrases)) return this.setState(state$1);
    ...
    */ get themeClasses() {
    ...
    mountStyles() {
        ...
        styleMod.StyleModule.mount(this.root, this.styleModules.concat(baseTheme$1).reverse(), nonce ? {
            nonce
        } : undefined);
    }
    readMeasured() {
...
*/ EditorView.cspNonce = state.Facet.define({
...
*/ const keymap = state.Facet.define({
...
function rectanglesForRange(view, className, range) {
    ...
    if (visualStart && visualEnd && visualStart.from == visualEnd.from && visualStart.to == visualEnd.to) {
        ...
        else if (top.bottom < bottom.top && view.elementAtHeight((top.bottom + bottom.top) / 2).type == exports.BlockType.Text) top.bottom = bottom.top = (top.bottom + bottom.top) / 2;
    ...
    function drawForLine(from, to, line) {
        ...
        function addSpan(from, fromOpen, to, toOpen, dir) {
            ...
            if (dir == exports.Direction.LTR) horizontal.push(ltr && fromOpen ? leftSide : fromCoords.left, ltr && toOpen ? rightSide : toCoords.right);
...
function rectangleFor(state$1, a, b) {
    ...
    if (a.off > MaxOff || b.off > MaxOff || a.col < 0 || b.col < 0) {
        ...
        for(let i = startLine; i <= endLine; i++){
            ...
            if (line.length <= endOff) ranges.push(state.EditorSelection.range(line.from + startOff, line.to + endOff));
        }
    } else {
...
function rectangleSelectionStyle(view, event) {
    ...
    return {
        ...
        get (event, _extend, multiple) {
            ...
            if (multiple) return state.EditorSelection.create(ranges.concat(startSel.ranges));
            else return state.EditorSelection.create(ranges);
        }
    };
}
/**
...
const lineNumberConfig = state.Facet.define({
    combine (values) {
        return state.combineConfig(values, {
            formatNumber: String,
            domEventHandlers: {}
        }, {
...
//# sourceMappingURL=node_modules_%40codemirror_view_dist_index_cjs_f22b03._.js.map